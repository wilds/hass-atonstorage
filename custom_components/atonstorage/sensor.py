...
def _create_entities(hass: HomeAssistant, entry: dict):
    ...
    sensors_selected = entry.data[CONF_MONITORED_VARIABLES]
            ...
            if isinstance(entity_description, AtonStorageSensorEntityDescription):
                ...
                entities.append(
                    AtonStorageIntegrationSensor(
                        ...
                        name=f"{controller.user} {entity_description.name}",
                        round_digits=2,
                        source_entity=f"sensor.{slugify(controller.user)}_{entity_description.source_sensor}",
...
class AtonStorageSensorEntity(CoordinatorEntity, SensorEntity):
    ...
    def __init__(
        ...
        self._attr_name = f"{controller.user} {self.entity_description.name}"
        self._attr_unique_id = f"{controller.serial_number}_{self.entity_description.key}"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, "AtonStorage " + controller.user)},
            name="Fotovoltaico " + controller.user,
